<template>
  <section>
    ...
    <div class="pub-mod-block" v-if="!state">
      <a class="label" v-if="list.length>0" :href="serverBaseUrl+'/export_message_list?id='+id">导出表格</a>
      <el-table :data="list.slice((currentPage-1)*pagesize,currentPage*pagesize)" v-loading="loading">
        ...
        <el-table-column prop="m" label="信息来源" width="150"></el-table-column>
        <el-table-column prop="t" label="信息内容"></el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    async getList () {
      const res = await this.$api.table.message({id: this.id})
      let data = res.data
      this.loading = false
      if (data.length && data[0]._message_list_json_array === 1) {
        this.state = 1
      } else {
        this.state = 0
      }
      if (data.length > 0 && JSON.parse(data[0]._message_list_json_array).length > 0) {
        this.list = JSON.parse(data[0]._message_list_json_array)
